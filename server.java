...
public class server extends JFrame{
		...
		userText.addActionListener(
		 new ActionListener() { 
			 public void actionPerformed(ActionEvent event) { //this method is called after hitting enter
				 ...
				 userText.setText(""); //this line v imp. as soon as u send the message. the typebox needs to become empty, so that we can type in another message. 
	...
	private void waitForConnection() throws IOException{ 
		...
		connection=server.accept(); //accepts the connection to socket. waits till someone is connected( loop keeps running prev)
		                            //startRunning() keeps running again and again. as soon as it connects,this creates a socket. 
	...
	private void setupStreams() throws IOException{ 
		output=new ObjectOutputStream(connection.getOutputStream()); //pathway to send stuff, assigns the output stream for the socket to 'output'
	...
	private void whileChatting() throws IOException {
		...
		sendMessage(message);
		abletoType(true); //allow users to write text. ( before editabale was set false when we werent connected) //USER
		do{ 
			try{
